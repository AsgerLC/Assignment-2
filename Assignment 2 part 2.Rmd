...
for(i in 1:5){
  #Removing NA's
  folded_1 <- folded_1[complete.cases(folded_1[,"Socialization"]),]
  #Make two subsets:
  test_df <- folded_1 %>%  subset(.folds == i)
  train_df <- folded_1 %>%  subset(.folds != i)
  # make a model where you regress the train_df
  mod <- lmerTest::lmer(CHI_MLU ~ Visit*Diagnosis + verbalIQ1 + MOT_MLU + types_MOT + (1|Child.ID) + (0 + Visit|Child.ID), data = train_df, control = lmerControl(optimizer = "nloptwrap", calc.derivs = FALSE,  optCtrl = list(ftol_abs = 1e-10, xtol_abs = 1e-10, maxeval=10000)))
  #predict "test_df" based on "mod"
  prediction <- predict(mod, test_df, allow.new.levels = TRUE)
